...
public function store(Request $request, Stage $stage)
{
    ...
    // 🔥 PROCESS EXAMPLES (shared across all types) - Add this helper function
    $processExamples = function() use ($request) {
        $examples = [];
        $examplesInput = $request->input('content.examples', []);
    ...
    if ($validated['type'] === 'drag_drop') {
        ...
        // 🔥 ADD EXAMPLES
        $content['examples'] = $processExamples();
