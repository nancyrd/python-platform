<x-app-layout>
  <x-slot name="header">
    <div class="levels-header-container">
      <div class="flex items-center">
        <div class="levels-icon-wrapper"><i class="fas fa-list-check"></i></div>
        <div class="ml-4">
          <h2 class="levels-title">Create Assessment</h2>
  ...
  <div class="levels-container" style="min-height: calc(100vh - 200px);">
    <div class="levels-table-container">
      <div class="table-header">
        <h3 class="table-title">New Assessment</h3>
        <p class="table-description">Attach a pre/post assessment to this stage</p>
      </div>
      ...
      <div class="form-wrapper">
        ...
        <form method="POST" action="{{ route('admin.stages.assessments.store', $stage) }}" id="createAssessmentForm">
          @csrf
          ...
          <div class="form-group">
            ...
            <select name="type" class="form-select" required>
              ...
              <option value="pre"  {{ old('type')==='pre'  ? 'selected' : '' }}>Pre-assessment</option>
              <option value="post" {{ old('type')==='post' ? 'selected' : '' }}>Post-assessment</option>
            </select>
            <div class="form-text">Only one pre and one post assessment are allowed per stage.</div>
            @error('type') <div class="form-text" style="color:#b91c1c">{{ $message }}</div> @enderror
          </div>

          <div class="form-group">
            <label class="form-label">Title</label>
            <input type="text" name="title" class="form-control" value="{{ old('title') }}" placeholder='e.g., Pre: print() & Basic Types' required>
            @error('title') <div class="form-text" style="color:#b91c1c">{{ $message }}</div> @enderror
          </div>

          <div class="form-group">
            <label class="form-label">Time Limit (seconds)</label>
            <input type="number" name="time_limit" class="form-control" value="{{ old('time_limit', 480) }}" min="30" max="7200">
            @error('time_limit') <div class="form-text" style="color:#b91c1c">{{ $message }}</div> @enderror
          </div>

          <div class="form-group">
            <label class="form-label">Instructions (optional)</label>
            <textarea name="instructions" class="form-control" rows="3" placeholder="Answer all questions, then submit.">{{ old('instructions') }}</textarea>
              ...
              @foreach($oldQs as $idx => $q)
                <div class="question-item mb-4 p-3 border rounded">
                  ...
                  <div class="mb-2">
                    ...
                    <textarea name="questions[{{ $idx }}][options]" class="form-control" rows="3" placeholder="Option A&#10;Option B&#10;Option C" required>{{ $q['options'] ?? '' }}</textarea>
                    @error("questions.$idx.options") <div class="form-text" style="color:#b91c1c">{{ $message }}</div> @enderror
                  </div>

                  <div class="mb-2">
                    <label class="form-label">Correct Answer (optional – must match exactly one option)</label>
                    <input type="text" name="questions[{{ $idx }}][correct]" class="form-control" value="{{ $q['correct'] ?? '' }}" placeholder="Exact text of the correct option">
          ...
          <div class="form-actions">
            ...
            <a href="{{ route('admin.stages.edit', $stage) }}" class="btn-cancel">
  ...
  <script>
    function addQuestion(){
      ...
      el.innerHTML = `
        ...
        <div class="mb-2">
          <label class="form-label">Options (one per line or comma-separated)</label>
          <textarea name="questions[${index}][options]" class="form-control" rows="3" placeholder="Option A&#10;Option B&#10;Option C" required></textarea>
        </div>
        <div class="mb-2">
          <label class="form-label">Correct Answer (optional – must match exactly one option)</label>
          <input type="text" name="questions[${index}][correct]" class="form-control" placeholder="Exact text of the correct option">
        </div>
        <button type="button" class="btn-remove" onclick="removeQuestion(this)">Remove Question</button>
      `;
      wrap.appendChild(el);
  ...
  <style>
    ...
    * { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }
    ...
    .form-text{display:block;margin-top:.35rem;font-size:.9rem;color:#6b7280}
    ...
    .btn-add:hover{transform:translateY(-1px);box-shadow:var(--shadow-md);color:#fff}
    .btn-remove{background:#fee2e2;color:#991b1b;border-color:#fecaca}
    .btn-remove:hover{background:#fecaca;color:#991b1b;transform:translateY(-1px)}
    ...
    .btn-submit:hover{transform:translateY(-1px);box-shadow:var(--shadow-md);color:#fff}
    .btn-cancel{background:#e2e8f0;color:#64748b;border-color:#cbd5e1}
    .btn-cancel:hover{background:#cbd5e1;color:#475569;transform:translateY(-1px)}
    .question-item{background:var(--purple-50);border:1px solid var(--purple-200);border-radius:.75rem}
    @media (max-width:768px){ .levels-container{padding:1rem} .form-actions{flex-direction:column} }
  </style>
