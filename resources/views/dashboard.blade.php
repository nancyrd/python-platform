            ...
            <div class="achievement-summary d-none d-md-flex">
                <div class="text-center me-4">
                    <div class="fw-bold text-warning fs-4">
                        {{ collect($progressByStage)->sum(fn($p) => $p && $p->stars_per_level ? array_sum($p->stars_per_level) : 0) }}
                    </div>
                    <small class="text-muted">Total Stars</small>
                </div>
                <div class="text-center">
                    <div class="fw-bold text-success fs-4">
                        {{ collect($progressByStage)->filter(fn($p) => $p && $p->post_completed_at)->count() }}/{{ count($stages) }}
                    </div>
...
@php
    ...
    // ======= Map data (DEFINED ONCE) =======
    // Build points in a serpentine grid so they look like a path across the map
    $mapPoints = [];
    $cols = 3; // adjust grid width if you want
    foreach (collect($stages)->values() as $i => $s) {
        $row = intdiv($i, $cols);
        $col = $i % $cols;
        if ($row % 2 === 1) { $col = ($cols - 1) - $col; } // serpentine

        // 0..100 viewBox percentages
        $x = 12 + ($col * (76/($cols-1))); // ~12%..88%
        $y = 18 + ($row * 26);              // rows at 18%, 44%, 70%...
        ...
        $mapPoints[] = [
            ...
            'unlocked_to'  => (int)($p->unlocked_to_level ?? 0),
            'completed'    => (bool) ($p?->post_completed_at),
            'has_pre'      => (bool) ($p?->pre_completed_at),
            'unlocked'     => (bool) ($s->unlocked ?? false),
            'model'        => $s,
        ];
    }

    // Build the SVG path "M x y L x y ..."
    $pathD = '';
    foreach ($mapPoints as $i => $pt) {
        $pathD .= ($i === 0 ? 'M' : ' L') . ' ' . $pt['x'] . ' ' . $pt['y'];
    }
    if ($pathD === '') { $pathD = 'M 10 50 L 90 50'; } // fallback

    // Choose avatar target: first not-completed stage, else last
    $avatarIndex = 0;
    foreach ($mapPoints as $idx => $pt) {
        if (!$pt['completed']) { $avatarIndex = $idx; break; }
        if ($idx === count($mapPoints)-1) $avatarIndex = $idx;
    }
@endphp
...
.stage-marker{ position: absolute; transform: translate(-50%,-50%); z-index: 5; }
.stage-pin{
...
.stage-label{ margin-top: 6px; text-align:center; font-size:.8rem; font-weight:700; text-shadow: 0 1px 0 rgba(255,255,255,.6); }
.level-dots{ display:flex; gap:4px; justify-content:center; margin-top:4px; }
.level-dot{ width:6px; height:6px; border-radius:50%; background:#dee2e6; opacity:.8; }
...
.learning-map-container { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; position: relative; overflow-x: hidden; }
...
.stage-node { transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); cursor: pointer; position: relative; }
.stage-node:hover { transform: translateY(-5px) scale(1.05); }
.stage-node.completed { animation: pulse-success 2s ease-in-out infinite; }
.stage-node.unlocked { animation: pulse-primary 2s ease-in-out infinite; }
@keyframes pulse-success { 0% { box-shadow: 0 0 0 0 rgba(25, 135, 84, 0.4); } 70% { box-shadow: 0 0 0 15px rgba(25, 135, 84, 0); } 100% { box-shadow: 0 0 0 0 rgba(25, 135, 84, 0); } }
@keyframes pulse-primary { 0% { box-shadow: 0 0 0 0 rgba(13, 110, 253, 0.4); } 70% { box-shadow: 0 0 0 15px rgba(13, 110, 253, 0); } 100% { box-shadow: 0 0 0 0 rgba(13, 110, 253, 0); } }
.connector-line { height: 4px; background: linear-gradient(90deg, transparent, #6c757d, transparent); border-radius: 2px; position: relative; overflow: hidden; }
.connector-line.active { background: linear-gradient(90deg, transparent, #0d6efd, transparent); animation: flow 2s linear infinite; }
@keyframes flow { 0% { background-position: -100% 0; } 100% { background-position: 100% 0; } }
.stage-card { backdrop-filter: blur(10px); background: rgba(255, 255, 255, 0.95); border: 1px solid rgba(255, 255, 255, 0.2); transition: all 0.3s ease; position: relative; overflow: hidden; }
.stage-card:hover { transform: translateY(-8px); box-shadow: 0 20px 40px rgba(0,0,0,0.1); }
.stage-card::before { content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.4), transparent); transition: left 0.5s ease; }
...
.achievement-badge { position: absolute; top: -10px; right: -10px; width: 30px; height: 30px; background: linear-gradient(45deg, #ffd700, #ffed4a); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; animation: bounce 1s ease infinite alternate; }
@keyframes bounce { from { transform: translateY(0px); } to { transform: translateY(-5px); } }
.star-animation { display: inline-block; animation: twinkle 1.5s ease-in-out infinite alternate; }
@keyframes twinkle { from { opacity: 0.5; transform: scale(0.8); } to { opacity: 1; transform: scale(1.2); } }
.btn-game { background: linear-gradient(45deg, #667eea, #764ba2); border: none; color: white; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; transition: all 0.3s ease; position: relative; overflow: hidden; }
.btn-game:hover { transform: translateY(-2px); box-shadow: 0 10px 25px rgba(102, 126, 234, 0.4); color: white; }
.btn-game::before { content: ''; position: absolute; top: 50%; left: 50%; width: 0; height: 0; background: rgba(255, 255, 255, 0.2); border-radius: 50%; transform: translate(-50%, -50%); transition: width 0.6s, height 0.6s; }
.btn-game:hover::before { width: 300px; height: 300px; }
.mobile-stage-item { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border-left: 4px solid transparent; transition: all 0.3s ease; }
.mobile-stage-item.completed { border-left-color: #198754; }
.mobile-stage-item.unlocked { border-left-color: #0d6efd; }
.mobile-stage-item.locked { border-left-color: #6c757d; }
.tooltip-custom { background: rgba(0, 0, 0, 0.8); color: white; padding: 8px 12px; border-radius: 6px; font-size: 12px; position: absolute; z-index: 1000; white-space: nowrap; }
.learning-path { position: relative; z-index: 2; }
@media (max-width: 768px) { .achievement-summary { display: none !important; } }
    </style>
    ...
    <div class="learning-map-container py-4">
        ...
        <!-- === ADVENTURE MAP === -->
        <div class="adventure-map-wrap mb-5">
            <div class="adventure-map-bg"></div>
            <div class="badge-float">
                <i class="fa-solid fa-shoe-prints me-1"></i> Journey
            </div>

            <div class="adventure-map-pad">
                <svg id="mapSvg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice">
                    <path id="journeyPath" d="{{ $pathD }}"></path>
                    <path id="journeyPathActive" d="{{ $pathD }}"></path>
                </svg>

                <!-- Avatar -->
                <div id="mapAvatar" class="avatar" aria-label="Adventurer">🐍</div>

                <!-- Stage markers -->
                @foreach($mapPoints as $idx => $pt)
                    @php
                        $cls   = $pt['completed'] ? 'completed' : ($pt['unlocked'] ? 'unlocked' : 'locked');
                        $badge = $pt['completed'] ? '🏆' : ($pt['unlocked'] ? '🚀' : '🔒');
                        $linkEnabled = $pt['unlocked'];
                        $unlockedTo  = $pt['unlocked_to'];
                        $levels      = max(0, $pt['levels']);
                    @endphp

                    <div class="stage-marker" style="left: {{ $pt['x'] }}%; top: {{ $pt['y'] }}%;">
                        @if($linkEnabled)
                            <a class="stage-pin {{ $cls }}" href="{{ route('stages.enter', $pt['model']) }}" data-bs-toggle="tooltip" title="{{ $pt['title'] }}">
                                <span style="font-size:22px">{{ $badge }}</span>
                            </a>
                        @else
                            <div class="stage-pin {{ $cls }}" data-bs-toggle="tooltip" title="{{ $pt['title'] }}">
                                <span style="font-size:22px">{{ $badge }}</span>
                            </div>
                        @endif
                        <div class="stage-label">{{ $pt['title'] }}</div>
                        <div class="level-dots">
                            @for($i=1; $i<=min($levels,8); $i++)
                                @php
                                    $dotCls = $i <= $unlockedTo ? 'done' : ($i === $unlockedTo+1 ? 'unlock' : '');
                                @endphp
                                <span class="level-dot {{ $dotCls }}"></span>
                            @endfor
                            @if($levels > 8)
                                <span class="ms-1 small text-muted">+{{ $levels-8 }}</span>
                            @endif
                        </div>
                    </div>
                @endforeach
        ...
        <div class="container learning-path">
            <!-- Desktop/Tablet Horizontal Stage Map (kept) -->
            <div class="row justify-content-center mb-5">
                <div class="col-12">
                    <div class="stage-card rounded-4 p-4 shadow-lg">
                            ...
                            @foreach($stages as $i => $stage)
                                ...
                                <div class="text-center" style="min-width: 160px;">
                                    <div class="position-relative d-inline-block">
                                        <div class="stage-node {{ $isCompleted ? 'completed' : ($isUnlocked ? 'unlocked' : '') }} {{ $nodeClass }} rounded-circle d-flex align-items-center justify-content-center text-white shadow-lg"
                                    ...
                                    <div class="mt-3">
                                        @if($isUnlocked)
                                            <a href="{{ route('stages.enter', $stage) }}" class="btn btn-game btn-sm px-3 py-2 rounded-pill shadow-sm">
                                                <i class="fas fa-play me-1"></i>{{ $ctaLabel }}
                                            </a>
                                        @else
                        ...
                        <!-- Mobile Vertical Stage List (kept) -->
                        <div class="d-md-none">
                            @foreach($stages as $stage)
                                ...
                                <div class="mobile-stage-item {{ $statusClass }} rounded-3 p-3 mb-3 shadow-sm">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            @if($isUnlocked)
                                                <a href="{{ route('stages.enter', $stage) }}" class="btn btn-game btn-sm px-3 py-1 rounded-pill">{{ $ctaLabel }}</a>
                                            @else
                                                <button class="btn btn-secondary btn-sm px-3 py-1 rounded-pill" disabled>Locked</button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                @foreach($stages as $stage)
                    @php
                        ...
                        if($p && $p->stars_per_level) {
                            foreach($p->stars_per_level as $s) { $stars += (int) $s; }
                        }
                    ...
                    <div class="col-lg-6">
                        <div class="stage-card rounded-4 shadow-lg h-100 overflow-hidden">
                            ...
                            <div class="p-4">
                                ...
                                <div class="text-end">
                                    @if($isUnlocked)
                                        <a href="{{ route('stages.enter', $stage) }}" class="btn btn-game px-4 py-2 rounded-pill shadow-sm">
                                            <i class="fas fa-{{ $isCompleted ? 'redo' : 'play' }} me-2"></i>{{ $cardCta }}
                                        </a>
                                    @else
                                        <button class="btn btn-secondary px-4 py-2 rounded-pill" disabled>
                                            <i class="fas fa-lock me-2"></i>Complete Previous Stage
                                        </button>
                                    @endif
                                </div>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            tooltipTriggerList.map(el => new bootstrap.Tooltip(el, { trigger: 'hover focus' }));
            ...
            document.querySelectorAll('.progress-ring-circle').forEach(r => ringObserver.observe(r));
        ...
        (function(){
            ...
            const avatarIndex = {{ (int) $avatarIndex }};
            const stageCount = {{ count($mapPoints) }};
            const positions = [];
            for(let i=0;i<stageCount;i++){
                positions.push(stageCount===1 ? 0 : (i/(stageCount-1)));
            }

            function placeAvatarAt(t){
                const L = base.getTotalLength();
                const pos = base.getPointAtLength(L * t);
                avatar.style.left = pos.x + '%';
                avatar.style.top  = pos.y + '%';

                const delta = 0.01;
                const p2 = base.getPointAtLength(Math.min(L, Math.max(0, L * (t + delta))));
                const ang = Math.atan2(p2.y - pos.y, p2.x - pos.x);
                avatar.style.transform = `translate(-50%,-50%) rotate(${ang}rad)`;
            }

            const targetT = positions[Math.max(0, Math.min(avatarIndex, positions.length-1))] ?? 0;
            let t = 0;
            const step = 0.015;
            placeAvatarAt(0);
            const timer = setInterval(()=>{
                t += step;
                if(t >= targetT){ t = targetT; clearInterval(timer); }
                placeAvatarAt(t);
            }, 30);

            // enable tooltips for stage pins after DOM settles
            setTimeout(()=>{
                document.querySelectorAll('.stage-pin').forEach(el=>{
